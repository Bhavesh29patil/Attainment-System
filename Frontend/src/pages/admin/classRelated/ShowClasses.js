...
const ShowClasses = () => {
  ...
  const sclassColumns = [{ id: "name", label: "Academic Year", minWidth: 170 }];
    ...
    sclassesList
      .filter((item) => {
        return searchQuery.toLowerCase() === ""
          ? item
          : item.sclassName.toLowerCase().includes(searchQuery);
      })
      .map((sclass) => {
        return {
          name: sclass.sclassName,
    ...
    <>
      {loading ? (
        ...
        <>
          ...
          {/* Render table with filtered rows if search query exists */}
